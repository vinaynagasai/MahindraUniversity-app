...
import { Bell, AlertCircle, CheckCircle, Info, Calendar, CreditCard, BookOpen, FileText, RefreshCw } from 'lucide-react';
...
const getTypeIcon = (title) => {
  ...
  if (lowerTitle.includes('exam') || lowerTitle.includes('result') || lowerTitle.includes('mark') || lowerTitle.includes('doubt')) return <BookOpen size={20} />;
  if (lowerTitle.includes('permission') || lowerTitle.includes('leave')) return <FileText size={20} />;
  return <Bell size={20} />;
};
...
export const StudentNotifications = () => {
  ...
  const notifications = getNotificationsByRole('student');
  ...
  const handleRefresh = () => {
    ...
    if (stored) {
      ...
      const storedNotifications = parsedData.notifications || [];
      const studentNotifications = storedNotifications.filter(n => n.forRole === 'all' || n.forRole === 'student');
      console.log('Refreshed notifications:', studentNotifications.length);
    }
    setRefreshKey(k => k + 1);
  };
        ...
        <div className={styles.notificationsList}>
          {notifications.length > 0 ? (
            ...
            <Card className={styles.emptyCard}>
              ...
              <p className={styles.emptyHint}>Notifications sent by faculty will appear here</p>
            </Card>
          )}
        </div>
      </div>
